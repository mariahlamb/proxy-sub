...
def load_sub_yaml(path_yaml):
    logger.info(f"Loading YAML file from {path_yaml}")
...
def get_channel_http(channel_url):
    ...
    try:
        ...
        logger.error(f"{channel_url} {e}")
    finally:
        return url_list

def filter_base64(text):
    ss = ['ss://', 'ssr://', 'vmess://', 'trojan://']
    return any(i in text for i in ss)
        ...
        def start_check_url(url):
            ...
            for check_url in check_url_list:
                try:
                    ...
                    res = requests.get(check_url_string, timeout=15)  # 设置15秒超时防止卡死
                    if res.status_code == 200:
                        airport_list.append(url)
                        break
                except Exception as e:
                    logger.error(f"解析失败: {check_url_string} - {e}")
...
def sub_check(url, bar):
    ...
    with thread_max_num:
        def start_check(url):
            res = requests.get(url, headers=headers, timeout=10)  # 设置10秒超时防止卡死
            if res.status_code == 200:
                ...
                try:
                    ...
                    logger.error(f"Error processing subscription info: {e}")
                    try:
                        if 'proxies:' in res.text:
                            new_clash_list.append(url)
                    except Exception as e:
                        try:
                            text = base64.b64decode(res.text[:64])
                            if filter_base64(text.decode('utf-8')):
        ...
        try:
            ...
            logger.error(f"Subscription check failed for {url} - {e}")
        finally:
            bar.update(1)
